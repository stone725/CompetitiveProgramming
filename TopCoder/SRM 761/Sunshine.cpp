...
struct Sunshine {
  int totalDays;
  int rainyDays;
  string weatherReport(int _totalDays, int _rainyDays) {
    totalDays = _totalDays, rainyDays = _rainyDays;
    string res = "";
    int score = totalDays - rainyDays;
    vector<int> sunnyDays(rainyDays + 1, score / (rainyDays + 1));
    for(int i = 0; i < score % (rainyDays + 1); i++){
        sunnyDays[i]++;
    }
...
char run_testcase(int __no) {
 ...
 return do_test(totalDays, rainyDays, __expected, __no, __unknownAnswer);
}
...
char do_test(int totalDays, int rainyDays, string __expected, int __caseNo, bool __unknownAnswer) {
    setColor(COL_CYAN);
    cout << "Test " << __caseNo;
    setColor(COL_WHITE);
    cout << ": [" << pretty_print(totalDays) << ","<< pretty_print(rainyDays)  << "]" << endl;
 
    Sunshine *__instance = new Sunshine();
    string __result = __instance->weatherReport(totalDays, rainyDays);
    delete __instance;
